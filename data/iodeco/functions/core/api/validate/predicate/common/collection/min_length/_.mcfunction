#> iodeco:core/api/validate/predicate/common/collection/min_length/_
# @within function iodeco:core/api/validate/predicate/common/collection/_

#>
# @within function iodeco:core/api/validate/predicate/common/collection/min_length/**
    #declare score_holder $Value
    #declare score_holder $Length


# 検証前にフラグを立てておく
    execute if data storage iodeco:core {Inverted:false} run data modify storage iodeco:core Predicate.minLength.success set value true
    execute if data storage iodeco:core {Inverted:true } run data modify storage iodeco:core Predicate.not.minLength.success set value true
...
# 検証に失敗 -> ArgumentError を発生させる
    execute if data storage iodeco:temp {Success:false} run function iodeco:core/api/validate/predicate/common/collection/min_length/fail/_
