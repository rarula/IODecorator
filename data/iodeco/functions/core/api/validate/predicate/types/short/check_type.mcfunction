...
# 検証前にフラグを立てておく
    data modify storage iodeco: out.short.success set value true
...
## 戻り値を設定する
    execute if data storage iodeco:temp {Matched:false} run data modify storage iodeco: out.short.success set value false
    execute if data storage iodeco:temp {Matched:false} run data modify storage iodeco: out.short.error set from storage iodeco:util out.Error
...
# 型が一致する -> short以降の引数をコピーし、条件との合致を検証する
    execute if data storage iodeco:temp {Matched:true} run data modify storage iodeco:core Args set from storage iodeco:core Args.short
