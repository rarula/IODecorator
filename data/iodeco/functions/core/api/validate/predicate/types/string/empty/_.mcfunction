#> iodeco:core/api/validate/predicate/types/string/empty/_
...
# 検証する
    execute store success storage iodeco:temp Success byte 1.0 if data storage iodeco: in{value:""}
...
# 検証に失敗 -> ArgumentError を発生させる
    execute if data storage iodeco:temp {Success:false} run function iodeco:core/api/validate/predicate/types/string/empty/fail/_
