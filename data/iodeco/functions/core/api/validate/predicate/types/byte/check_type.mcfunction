...
# 検証前にフラグを立てておく
    data modify storage iodeco: out.byte.success set value true
...
## 戻り値を設定する
    execute if data storage iodeco:temp {Matched:false} run data modify storage iodeco: out.byte.success set value false
    execute if data storage iodeco:temp {Matched:false} run data modify storage iodeco: out.byte.error set from storage iodeco:util out.Error
